...
public class RdpController {
    ...
    public R ParFieldsForJSON(@RequestParam("dataSourceName") String dataSourceName, @RequestParam("dataSetType") String dataSetType, @RequestParam("parm") String parm, @RequestParam("parms") String parms, @RequestParam("value") String value) {
        try {
            ...
            List<DataParms> dataParms = new ArrayList<DataParms>();
            if (StringUtils.isNotBlank(parms)) {
                dataParms = JSONObject.parseArray(parms, DataParms.class);
            }
            ...
            if (dataMap != null && dataMap.size() > 0) {
                if ("1".equals(dataMap.get("flag"))) {
                    ...
                    List<TableColumnBean> list = designService.parFieldsForJSON(dsb, dataSetType, sqlStr, parm, dataParms);
    ...
    public R upload(MultipartFile file) {
        ...
        String filePathName = realPath + File.separator + fileName;
        try {
        ...
        if(uuIds!=null) {
            ...
            try {
                ...
                for (int i = 0; i < uuids.size(); i++) {
                    ...
                    byte[] f = FileUtil.readBytes(realPath + File.separator + uuids.get(i) + ".xml");
