...
function initSearchParm(uuid) {
    ...
    var loadpage = function (qa) {
        ...
        if (qa.css) {
            loadStyle(Base64Util.decode64(qa.css));
        }
        if (qa.js) {
            loadScript(Base64Util.decode64(qa.js));
        }
        if (qa.height) {
            $(".rt-parmlist").height(parseInt(qa.height) + 10);
        }
    }
    var sdata = {uuid: uuid};
    $.ajax({
        ...
        success: function (res) {
            ...
            if (data.searchParmJson.length <= 0) {
                ...
                $("#searchaddition").append('<input type="button" onclick="init(1)" value="查询" id="searchlist" />')
            }
            if(autoSub==1){
                init(1);
            }
