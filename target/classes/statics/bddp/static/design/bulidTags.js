...
function setTableStyle(box, prop) {
...
function bulidFilterInput(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-input" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterY(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-y" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterYM(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-ym" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterDate(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-date" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterYRang(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-y-rang" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterYMRang(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-ym-rang" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterDateRang(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><input type="text" class="tag-filter-date-rang" style="width: 100%;height: 100%;"/></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidFilterSearchbtn(prop, res) {
    var box = $('<div class="box" style="width: 100px;height: 40px;"><button class="tag-filter-searchbtn"  style="width: 100%;height: 100%;">查询</button></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    box.appendTo("#content");
    box.data("prop", prop);
...
function bulidTable(prop, res) {
    ...
    setTagsParts(box, parts, prop);
    setTableStyle(box,prop);
    return box;
}
function bulidHeadtable(prop, res) {
    var box = $('<div class="box" style="width: 480px;height: 320px;"><table class="tag-headtable" style="width:100%;height:100%;margin:0; padding:0;"></table></div>');
    var parts = prop.parts;
    setRectP(box, prop);
    delete prop.id;
    box.appendTo("#content");
...
function createChart(prop, oelem) {
    ...
    if (prop.optionsText && typeof (prop.optionsText) == "string") {
        prop.options = eval("(" + prop.optionsText + ")");
    }
    ...
    if (res) {
        ...
        $.each(tempObj, function (k, v) {
            if (options.radar.indicator[count]) {
                options.radar.indicator[count] = $.extend({}, options.radar.indicator[count], {
                    name: k,
                    max: max
                })
            } else {
...
function bulidMapData(elem, md5, mappath, res) {
    ...
    prop.optionsText = JSON.stringify(options, function (k, v) {
        if (typeof (v) == "function") {
            return "&" + v.toString().replace(/\s+/g, ' ').replace(/\n/g, "") + "&";
        }
        return v;
    }, 4);
...
function setChartTheme(elem, prop) {
    ...
    if (other && other.theme) {
        var file = "../../statics/bddp/static/charts-theme/" + other.theme + ".json";

        getJSONFileData(file, function (data) {
            var theme = data;
            echarts.registerTheme(other.theme, theme);
            var tempCahrt = echarts.init(elem.find(".tag-charts").empty()[0], other.theme);
