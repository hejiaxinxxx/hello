      ...
      event: {
        ondestroy: {
          before: function (layxWindow, winform, params, inside, escKey) {
            if (params.name) {
              $("#datalink").val("../../bddpConfig/" + params.id + "/data/" + params.name);
            }
          }
        }
      }
    ...
    layx.iframe('localsiteforjson', 'JSON数据文件选择', './component/BddpDataPage.html', {
      event: {
        ondestroy: {
          before: function (layxWindow, winform, params, inside, escKey) {
            if (params.name) {
              $("#globalDataUrl").val("../../bddpConfig/" + params.id + "/data/" + params.name);
            }
          }
        }
      }
        ...
        callback: function (id, button, event) {
          ...
          if (value) {
            ...
            saveBddpData(bdData, function (res) {
              if (res.code == 0) {
                console.log("新建成功");
                history.pushState({}, value, "design.html?uuid=" + bdData.content.id);
  ...
  getJSONFileData("../../bddp/getBddpFolders", function (res) {
    if (res.code == 0) {
      ...
      for (var i = 0; i < fileArrs.length; i++) {
        var file = fileArrs[i];
        var li = $('<li class="list-item"><div class="list-item-image"></div><span class="list-item-name" title="' + file.name + '">' + file.bdname + '</span><div class="fire"></div> <a href="#" class="x fa fa-pencil"></a><a href="#" class="y fa fa-share-alt"></a></li>');
        li.append('<i class="list-item-del fa fa-close"></i>');
        li.append('<i class="list-item-copy fa fa-copy"></i>');

        li.find(".list-item-image").css("background-image", "url(../../bddpshow/bgi/" + file.name + "?date=" + new Date().getTime() + ")")
        $("#file-list").append(li);
        li.data("path", file.path);
        li.data("name", file.name);
        li.data("bdname", file.bdname);
        li.find(".fa-pencil").bind("click", function () {
          history.pushState({}, $(this).parent(".list-item").data("bdname"), "design.html?uuid=" + $(this).parent(".list-item").data("name"));
          importData($(this).parent(".list-item").data("path"));
        });
        li.find(".fa-share-alt").bind("click", function () {
          window.open('../../bddpshow/show/' + $(this).parent(".list-item").data("name"));
        });
        li.find(".list-item-del").bind("click", function () {
          var path = $(this).parent(".list-item").data("path");
          var pObj = $(this).parent(".list-item");
          layx.confirm('消息提醒', '确认要删除“' + $(this).parent(".list-item").data("bdname") + '”？', function (id) {
...
function openOldbd() {
  getJSONFileData("../../bddp/getBddpFiles", function (res) {
    if (res.code == 0) {
      ...
      for (var i = 0; i < fileArrs.length; i++) {
        var file = fileArrs[i];
        var li = $('<li class="list-item"><div class="list-item-image"></div><span class="list-item-name" title="' + file.name + '">' + file.name + '</span></li>');
        $("#file-list").append(li);
        li.data("path", file.path);
...
function savebd(callback) {
  ...
  $("#hide-boxs").children(".box-hide").each(function () {
    var prop = $(this).data("prop");
      ...
      callback: function (id, button, event) {
        ...
        html2canvas($("#content")[0], {
          ...
          try {
            ...
            $("#content").css("transform", "scale(" + $("#content").data("zoom") + ")");
...
function initDataConfig() {
  ...
  $("#datalinkSaveBtn").bind("click", function () {
    ...
    if ($(".box-selected").length > 0) {
      ...
      var prop = $(".box-selected").data("prop");
      if (dataFormVal == 4) {
        data.link = getConfigProp("rdpserver") + "ex/ser/dataset/sqlgroupresult?";
        var columns = data.series.map(function (obj) {
          return obj.keyname.split("data.")[1];
        }).join(",");
        var groups = data.dimension.map(function (obj) {
          return obj.keyname.split("data.")[1];
        }).join(",");
        var series = data.classify.map(function (obj) {
          return obj.keyname.split("data.")[1];
        }).join(",");
  ...
  $(".data-dimension,.data-series,.data-classify").droppable({
    ...
    drop: function (event, ui) {
      ...
      var tempHtml = $('<div class="data-item" title="' + keyName + '" data-keyname="' + keyName + '" data-displayname="' + keyName + '"><span class="desc">' + keyName + '</span><i class="fa fa-edit "></i><i class="fa fa-close"></i></div>');
...
function getDataSetsData(data) {
  reqServerControllerParms("ex/ser/dataset/result", {
    dtId: data.dtId
  }, function (res) {
    console.log(res);
    if (res.code == 0) {
...
function getProp(elem, flag) {
  ...
  if (currBox && currBox.is(elem) && !flag) {
    ...
    if (!result) {
      result = executeSQLAsObject("SELECT * FROM tags WHERE tagName='" + type + "'");
    ...
    if (!res) {
      res = executeSQL("SELECT pId,id,name,keyName,tagprop.tagId FROM	propitems,tagprop where propitems.type ISNULL and tagprop.propId = propitems.id AND tagprop.tagId =" + result.id);
    ...
    if (data) {
      ...
      $.each(dimension, function (i, dm) {
        var tempHtml = $('<div class="data-item" title="' + dm.keyname + '" data-keyname="' + dm.keyname + '" data-displayname="' + dm.displayname + '"><span class="desc">' + dm.displayname + '</span><i class="fa fa-edit "></i><i class="fa fa-close"></i></div>');
        tempHtml.find("i.fa-edit").bind("click", function () {
          var input = $('<input type="text" class="data-item-input" />');
          $(this).parent().append(input);
          input.bind("blur", function () {
    ...
    if (node.keyName == "series") {
      ...
      if (data && options.series) {
        $.each(options.series, function (j, sub) {
          var li = $(' <li><a href="#' + node.keyName + "-" + j + '">' + node.name + j + '</a></li>');
          li.data("node", node);
          li.data("lv", lv);
          ul.append(li);
          var div = $('<div data-keyname="' + node.keyName + '" data-index="' + j + '" id="' + node.keyName + "-" + j + '"></div>');
      ...
      var li = $(' <li><a href="#' + node.keyName + '">' + node.name + '</a></li>');
      li.data("node", node);
      li.data("lv", lv);
      ul.append(li);
      var div = $('<div data-keyname="' + node.keyName + '"  id="' + node.keyName + '"></div>');
      if (node.children && node.children.length > 0) {
        bulidTabs(div, node.children, lv + 1, prop);
      }
      box.append(div);
...
function bulidProp(elem, propId, tagId) {
  ...
  if (!res) {
    var sqlStr = "select propitems.*,a.tagId FROM propitems LEFT JOIN (SELECT * FROM tagprop WHERE tagId=" + tagId + ") a ON a.propId =propitems.id WHERE a.tagId NOTNULL and propitems.pId = '" + propId + "'" + " ORDER BY sqen";
